...
module.exports = {
	"parse": (args) => {
		...
		let pingedUser
		if (!((msg.content.toLowerCase().startsWith(`/био`) || (msg.content.toLowerCase().startsWith(`/bio`))))) return

		try {
			let args = msg.content.slice(`/био`).split(/ +/);

			{ // нахождение pingedUser
				if (args.length >= 2) {
					pingedUser = args[1]
				...
				if (pingedUser.includes('<@')) {
					...
					msg.reply('Не найдено пользователей! Используйте упомянание или его id, чтобы указать пользователя')
				}
			};
				...
				try {
					client.users.fetch(pingedUser).then(myUser => {
						...
						if (fs.existsSync(colorPath)) {
							COLOR = fs.readFileSync(colorPath, "utf8");
						} else {
							COLOR = "#ff8800"
						}
